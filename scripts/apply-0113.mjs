// Applique la migration 0113 (carte réelle du Fournil) via la fonction PG
...
//
// Usage : node scripts/apply-0113.mjs
...
const BAK = 'backups/fournil-avant-0113.json'

// ─── 1. Sauvegarde avant écriture ───────────────────────────────────
const { data: avant, error: eSel } = await sb.from('recettes').select('*').eq('tag_destination', 'FOURNIL')
...
// ─── 2. Application ─────────────────────────────────────────────────
const sql = readFileSync('supabase/migrations/0113_carte_fournil_reelle.sql', 'utf8')
...
console.log('✓ migration 0113 appliquée —', JSON.stringify(data))
