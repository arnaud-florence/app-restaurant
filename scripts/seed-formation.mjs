...
const MANUELS = [
  ...
  { fichier: '02-serveur.md',        titre: 'Manuel Serveur',        poste: 'serveur',        duree: 25, ordre: 2 }, // +5 min (section multi-canal)
  { fichier: '03-cuisinier.md',      titre: 'Manuel Cuisinier',      poste: 'cuisinier',      duree: 30, ordre: 3 }, // +5 min (section multi-canal)
  { fichier: '04-pizzaiolo.md',      titre: 'Manuel Pizzaiolo',      poste: 'pizzaiolo',      duree: 25, ordre: 4 },
  { fichier: '05-barman.md',         titre: 'Manuel Barman',         poste: 'barman',         duree: 30, ordre: 5 },
  { fichier: '06-receptionniste.md', titre: 'Manuel Réceptionniste', poste: 'receptionniste', duree: 25, ordre: 6 },
  { fichier: '07-second.md',         titre: 'Manuel Second',         poste: 'second',         duree: 35, ordre: 7 }, // +5 min
  { fichier: '08-plonge.md',         titre: 'Manuel Plonge',         poste: 'plonge',         duree: 15, ordre: 8 },
  { fichier: '09-snack.md',          titre: 'Manuel Snack/Comptoir', poste: 'snack',          duree: 25, ordre: 9 },
  { fichier: '10-livreur.md',        titre: 'Manuel Livreur',        poste: 'livreur',        duree: 15, ordre: 10 },
]
...
const QUIZ = {
  ...
  snack: [
    { q: "Quel badge couleur correspond à une commande venant de la BORNE ?",
      choix: ["Bleu 🪑", "Violet 🛒", "Émeraude 🌐", "Rouge 🛍"],
      bonne: 3, ex: "Rouge 🛍 = BORNE. Les 4 sources ont chacune une couleur distincte sur les tickets cuisine." },
    { q: "Une commande COMPTOIR créée sans paiement immédiat : que se passe-t-il ?",
      choix: ["Elle part en cuisine aussitôt", "Elle reste en attente_paiement_comptoir, invisible cuisine", "Elle est annulée", "Elle est facturée plus tard"],
      bonne: 1, ex: "Pas de paiement = pas de cuisine. Réflexe d'or : toujours encaisser AVANT que la commande parte en cuisine." },
    { q: "Sur quelle page tu encaisses une commande BORNE en attente de paiement comptoir ?",
      choix: ["/caisse", "/bar", "/emporter", "/serveur"],
      bonne: 2, ex: "/emporter centralise les commandes BORNE/COMPTOIR à encaisser, en plus du retrait ONLINE." },
    { q: "Pour une commande COMPTOIR, à quel moment demander la fidélité ?",
      choix: ["Jamais", "À chaque transaction, avant l'encaissement", "Seulement si le client demande", "Une fois par mois"],
      bonne: 1, ex: "Demande systématique = 80 % de prises CRM. Le client met 5 sec à donner son tel ou son nom." },
    { q: "Un client à la borne échoue à payer en NFC 3 fois de suite. Que fais-tu ?",
      choix: ["Tu attends qu'il abandonne", "Tu proposes le paiement au comptoir, tu vas à l'aide", "Tu redémarres la borne", "Tu annules la commande"],
      bonne: 1, ex: "3 échecs NFC = bascule comptoir + assistance physique. Surveille `borne_nfc_echecs` sur /admin/borne." },
    { q: "Comment marquer une commande ONLINE retirée par le client ?",
      choix: ["Elle se marque seule", "Sur /emporter → bouton ✅ Retiré, après remise", "Sur /caisse", "Aucune action requise"],
      bonne: 1, ex: "Tu marques manuellement après remise. Sinon historique flou + manager ne sait pas si tout est OK." },
    { q: "Catalogue borne : qu'est-ce qui est INTERDIT à la vente ?",
      choix: ["Snacks", "Pizzas", "Boissons soft", "Alcools forts (vins, cocktails)"],
      bonne: 3, ex: "Borne refuse alcool fort = protection mineurs. Tu sers au comptoir avec vérification ID." },
    { q: "À l'ouverture, qu'est-ce que tu vérifies en premier sur la borne ?",
      choix: ["Le prix des produits", "Allumée + NFC OK + papier ticket OK + page d'accueil", "Les ventes d'hier", "Rien"],
      bonne: 1, ex: "Check matériel = pas de surprise pendant le service. NFC + papier sont les pannes les plus fréquentes." },
    { q: "Pourquoi peser séparément les emballages en fin de service ?",
      choix: ["Pour les recycler", "Loi AGEC + suivi gâchis spécifique snack (très emballeur)", "C'est facultatif", "Pour la déco"],
      bonne: 1, ex: "Snack = volume emballages important. Pesée séparée révèle un poste d'amélioration (vrac, consigne)." },
    { q: "Sur /admin/borne (lecture), quel seuil d'échec NFC déclenche une alerte ?",
      choix: ["1 %", "5 %", "10 %", "50 %"],
      bonne: 2, ex: "Au-delà de 10 % d'échecs NFC, problème technique probable (terminal, antenne). Préviens Arnaud." },
  ],
  livreur: [
    { q: "À quel moment cliquer le bouton '🛵 Partir' dans /livreur ?",
      choix: ["Quand la cuisine commence", "Quand tu as le sachet dans tes mains", "Avant de partir au resto", "Après livraison"],
      bonne: 1, ex: "Avec le sachet en main = pas d'oubli. '🛵 Partir' bascule la commande en 'en_livraison'." },
    { q: "Tu sonnes chez le client, personne ne répond. Que fais-tu en premier ?",
      choix: ["Tu laisses le paquet devant la porte", "Tu appelles le client", "Tu rentres au resto", "Tu sonnes plus fort"],
      bonne: 1, ex: "Appel = #1 réflexe. Souvent le client est à la douche ou n'a pas entendu. Pas de paquet sans contact." },
    { q: "Quelle source de commande s'affiche sur ta page /livreur ?",
      choix: ["TABLE uniquement", "ONLINE avec mode_retrait='livraison' uniquement", "Toutes", "COMPTOIR"],
      bonne: 1, ex: "Tu ne vois QUE les ONLINE livraison. Tout le reste est invisible pour toi." },
    { q: "À quel moment cliquer le bouton '✓ Livrée' dans l'app ?",
      choix: ["En rentrant au resto", "Immédiatement après avoir remis le sachet", "Plus tard, quand tu as le temps", "Le manager le fait"],
      bonne: 1, ex: "Immédiatement = source unique de vérité. '✓ Livrée' passe la commande en 'retire_par_client' ou 'encaisse'." },
    { q: "Le client refuse la livraison (mauvais plat). Que fais-tu ?",
      choix: ["Tu insistes", "Tu reprends le sachet + tu préviens le manager via /equipes (sans cliquer Livrée)", "Tu cliques '✓ Livrée' quand même", "Tu jettes le sachet"],
      bonne: 1, ex: "Sachet repris + manager informé. Lui décide du geste commercial (re-livraison, remboursement) depuis l'admin." },
    { q: "Tu as 3 livraisons dans 3 zones différentes en 30 min. Que fais-tu ?",
      choix: ["Tu attends que toutes soient prêtes", "Tu pars dès la 1ʳᵉ prête, regroupes par zone géographique", "Tu pars chargé avec les 3 ensemble", "Tu refuses"],
      bonne: 1, ex: "Timing serré = départ dès la 1ʳᵉ prête + regroupement géographique. Sinon retard cumulatif." },
    { q: "Où trouves-tu l'allergie d'un client habitué ?",
      choix: ["Dans /admin/clients fiche client", "Sur le sachet", "Tu demandes au cuisinier", "Tu devines"],
      bonne: 0, ex: "Fiche client = allergies mémorisées. Utile si le client te demande conseil à la remise." },
    { q: "Tu es en retard de 20 min sur le créneau. Premier réflexe ?",
      choix: ["Tu te dépêches sans rien dire", "Tu appelles le client (📞) ou cliques '📧 Prévenir client du retard'", "Tu cliques '✓ Livrée' à l'avance", "Tu attends d'arriver"],
      bonne: 1, ex: "Communication AVANT le retard = client conciliant. L'app propose un email retard auto si le client a donné son email." },
    { q: "Le sac isotherme garde la pizza chaude combien de temps maximum ?",
      choix: ["10 min", "30-45 min", "2 heures", "Indéfiniment"],
      bonne: 1, ex: "30-45 min en sac correct. Au-delà la pizza refroidit. Plan ta tournée en conséquence." },
    { q: "Tu rentres au resto, ta tournée est finie. Que vérifies-tu sur /livreur ?",
      choix: ["Rien", "Que la section '🛵 En tournée' est vide (tout est passé en 'livrées')", "Tu fermes l'app", "Tu vas voir le manager"],
      bonne: 1, ex: "Clôture propre = section 'En tournée' vide. Une commande qui y reste = oubli ou incident à régulariser." },
  ],
}
